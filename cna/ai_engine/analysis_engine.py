...
# Severity thresholds
_CRITICAL_IDS = {
    R_AWS_SG_UNRESTRICTED_SSH, R_AWS_SG_UNRESTRICTED_RDP,
    R_AWS_SG_UNRESTRICTED_ALL, R_AZ_FW_THREAT_INTEL_NOT_DENY,
}
_HIGH_IDS = {
    R_AWS_VPC_FLOW_LOGS_DISABLED, R_AZ_SUBNET_NO_NSG,
    R_AZ_VNET_NO_FLOW_LOGS, R_AZ_APPGW_WAF_DISABLED,
}
...
class AnalysisEngine:
    ...
    def _emit(self, finding: Finding) -> None:
        ...
        logger.debug("Finding emitted: %s on %s [%s]",
                     finding.rule_id, finding.resource_id, finding.severity.value)
    ...
    def _analyze_azure_subscription(
        self, sub_topo: AzureSubscriptionTopology
    ) -> None:
    ...
    def run(
        ...
        if aws_topology and self.opts.load_aws:
            ...
            for i, region_topo in enumerate(aws_topology.regions):
                if progress:
                    progress("aws", i + 1, total,
                             f"{region_topo.account_id}/{region_topo.region}")
        ...
        if critical_findings:
            logger.warning(
                ...
                engagement_id, len(critical_findings)
            )
        ...
        report = FindingsReport(
            ...
            critical_count=len([f for f in self._findings
                                 if f.severity == FindingSeverity.CRITICAL]),
            high_count=len([f for f in self._findings
                            if f.severity == FindingSeverity.HIGH]),
            medium_count=len([f for f in self._findings
        ...
        if not self.opts.dry_run:
            ...
            logger.info(
                ...
                engagement_id,
                report.total_count, report.critical_count,
