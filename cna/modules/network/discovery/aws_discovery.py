...
class AWSDiscovery:
    ...
    def _list_org_accounts(self) -> list[AWSAccount]:
        ...
        while True:
            ...
            for a in resp.get("Accounts", []):
                if a["Status"] != "ACTIVE":
                    logger.info("Skipping %s account %s (%s)",
                                a["Status"], a["Id"], a["Name"])
                    continue
    ...
    def _get_enabled_regions(self, session: boto3.Session) -> list[str]:
        ...
        resp = with_retry(ec2.describe_regions, Filters=[{"Name": "opt-in-status",
    ...
    def _discover_region(self, session: boto3.Session, account_id: str,
                         region: str) -> AWSRegionTopology:
    ...
    def _collect_vpcs(self, ec2, account_id: str, region: str) -> list[VPC]:
        ...
        for page in paginator.paginate():
            for v in page["Vpcs"]:
                ...
                vpcs.append(vpc)
                logger.debug("Collected VPC %s (%s) in %s/%s",
                             v["VpcId"], name or "unnamed", account_id, region)
        return vpcs
    ...
    def _collect_peering(self, ec2, vpc_id: str) -> list[VpcPeeringConnection]:
        ...
        for role in ["requester-vpc-info.vpc-id", "accepter-vpc-info.vpc-id"]:
            ...
            for page in paginator.paginate(
                Filters=[{"Name": role, "Values": [vpc_id]}]
            ):
                for p in page["VpcPeeringConnections"]:
                    ...
                    peerings.append(VpcPeeringConnection(
    ...
    def _collect_tgw_attachments(self, ec2, tgw_id: str) -> list[TGWAttachment]:
        ...
        for page in paginator.paginate(
            ...
            for a in page["TransitGatewayAttachments"]:
                ...
                attachments.append(TGWAttachment(
    ...
    def _collect_dx(self, dc, account_id: str, region: str) -> list[DirectConnectConnection]:
        ...
        try:
            ...
            logger.warning("DirectConnect describe_connections failed in %s: %s",
    ...
    def _collect_vpn_gateways(self, ec2, account_id: str, region: str) -> list[VpnGateway]:
        ...
        try:
            ...
            logger.warning("describe_vpn_gateways failed in %s: %s",
                           region, e.response["Error"]["Message"])
        return vgws
    ...
    def _infer_target_type(target: str) -> str:
        if not target:
            return "unknown"
        prefixes = [
            ("igw-", "igw"), ("nat-", "nat"), ("tgw-", "tgw"),
            ("pcx-", "pcx"), ("vgw-", "vpgw"), ("eni-", "eni"),
            ("lgw-", "lgw"), ("i-", "instance"),
        ]
    ...
    def run(self) -> AWSTopology:
        ...
        if not accounts:
            accounts = [AWSAccount(account_id=mgmt_account_id,
        ...
        for account in accounts:
            ...
            logger.info("[%s] Discovering account %s (%s)",
                        engagement_id, account_id, account.account_name or "")
            ...
            for region in regions:
                ...
                logger.info("[%s] %s/%s: %d VPCs%s",
                            engagement_id, account_id, region, vpc_count, blocked)

        logger.info("[%s] AWS discovery complete. %d regions collected.",
                    engagement_id, len(topology.regions))
        return topology
