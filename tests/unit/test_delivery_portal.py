...
class TestPortalGenerator:
    def test_renders_html_with_entries(self, tmp_path):
        ...
        entries = [
            PortalEntry(label="Executive Report", format="pdf", lang="en",
                        size_bytes=102400, rendered_at="2026-03-05T00:00:00Z",
                        download_url="https://example.com/file.pdf", is_stale=False),
        ]
        gen = PortalGenerator()
    ...
    def test_stale_badge_rendered_when_stale(self, tmp_path):
        ...
        entries = [
            PortalEntry(label="Executive", format="pdf", lang="en",
                        size_bytes=1024, rendered_at="2026-03-01T00:00:00Z",
                        download_url="https://x.com/f.pdf",
                        is_stale=True, stale_reason="Rendered before re-analysis"),
        ]
        gen = PortalGenerator()
        out = tmp_path / "index.html"
        gen.generate(manifest=manifest, entries=entries,
                     published_at="2026-03-05T00:00:00Z",
                     expires_at="2026-03-12T00:00:00Z",
                     output_path=out)
        content = out.read_text()
    ...
    def test_current_badge_when_not_stale(self, tmp_path):
        manifest = _make_manifest(1)
        entries = [
            PortalEntry(label="Technical", format="pdf", lang="en",
                        size_bytes=2048, rendered_at="2026-03-05T00:00:00Z",
                        download_url="https://x.com/t.pdf", is_stale=False),
        ]
        gen = PortalGenerator()
        out = tmp_path / "index.html"
        gen.generate(manifest=manifest, entries=entries,
...
class TestAccessManager:
    ...
    def test_is_expired_true_when_past_expiry(self):
        ...
        record = AccessRecord(
            ...
            ttl_hours=1, storage_location="s3://bucket", deliverable_count=1,
        )
    ...
    def test_build_record_caps_ttl(self):
        record = AccessManager.build_record(
            engagement_id="test", cloud="aws",
            storage_location="s3://b", deliverable_count=3,
            ttl_hours=9999,
        )
