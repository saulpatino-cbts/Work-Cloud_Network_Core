...
# Create the job via `az containerapp job create --yaml`, for two reasons:
#  1. Identity: the previous raw `az rest` PUT set the user-assigned identity only
#     at the resource level, which on a Consumption environment left it bound to the
#     job definition but NOT surfaced to the running container — the managed-identity
#     token endpoint then 500'd on every request. Letting the containerapp CLI create
#     the job (here via --yaml) configures identity availability correctly.
#  2. The "-c" argument: az containerapp job's --args flag CANNOT express a value
#     starting with "-" ("-c" is parsed as an unknown CLI flag -> "unrecognized
#     arguments: -c ..."). The YAML manifest carries command/args as plain list
#     items, immune to CLI arg parsing. (--yaml ignores all other create flags, so
#     identity/registry/env all live in the file.)
