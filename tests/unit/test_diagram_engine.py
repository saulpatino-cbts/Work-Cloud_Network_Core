    ...
    return AWSRegionTopology(
        ...
        vpcs=[
            VPC(
                ...
                cidr="10.0.0.0/16",
                subnets=[
                    Subnet(id="subnet-a", name="public-a", cidr="10.0.1.0/24",
                           az="us-east-1a", subnet_type=SubnetType.PUBLIC),
                    Subnet(id="subnet-b", name="private-b", cidr="10.0.2.0/24",
                           az="us-east-1b", subnet_type=SubnetType.PRIVATE),
                ],
            )
        ],
    )
...
def blocked_region():
    return AWSRegionTopology(
        account_id="222222222222", region="ap-southeast-1",
        discovery_blocked=True, block_reason="SCP denied ec2:DescribeVpcs"
    )
        ...
        vnets=[
            VNet(
                ...
                subnets=[
                    ...
                    AzureSubnet(id="sn-002", name="db-subnet", address_prefix="10.1.2.0/24",
                                nsg_id="/nsg/db-nsg"),
                ],
            )
        ],
...
class TestVpcTopology:
    ...
    def test_large_vpc_many_subnets(self):
        ...
        subnets = [
            Subnet(id=f"sn-{i}", cidr=f"10.0.{i}.0/24", az="us-east-1a",
...
class TestVnetTopology:
    ...
    def test_empty_subscription_produces_note(self):
        sub = AzureSubscriptionTopology(
            subscription_id="sub-empty", tenant_id="t-001", vnets=[]
        )
    ...
    def test_blocked_subscription_raises(self):
        sub = AzureSubscriptionTopology(
            subscription_id="sub-blocked", tenant_id="t-001",
            discovery_blocked=True, block_reason="Azure Policy denied"
        )
        with pytest.raises(ValueError, match="Discovery was blocked"):
...
class TestVwanTopology:
    def test_produces_valid_xml(self):
        vwan = AzureVWan(
            id="/vwan/prod-vwan", name="prod-vwan",
            resource_group="rg-conn", sku="Standard",
            hubs=[
                AzureVHub(
                    id="/hub/us-hub", name="us-hub", location="eastus",
                    resource_group="rg-conn", address_prefix="10.100.0.0/23",
    ...
    def test_empty_vwan_produces_note(self):
        vwan = AzureVWan(
            id="/vwan/empty", name="empty-vwan",
            resource_group="rg-conn", sku="Standard", hubs=[]
        )
        xml = generate_vwan_topology(vwan)
...
class TestMermaidGenerators:
    ...
    def test_azure_mg_hierarchy_starts_with_flowchart(self):
        topology = AzureTopology(
            ...
            management_groups=[
                ManagementGroup(
                    id="mg-root", name="root", display_name="Root",
                    child_mg_ids=["mg-prod"],
                ),
                ManagementGroup(
                    id="mg-prod", name="prod", display_name="Production",
                    parent_id="mg-root",
